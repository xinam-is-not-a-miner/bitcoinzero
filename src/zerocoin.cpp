...
bool CheckZerocoinFoundersInputs(const CTransaction &tx, CValidationState &state, int nHeight, bool fTestNet) {
    // Check for founders inputs
        if (sporkManager.IsSporkActive(SPORK_7_FEE_CHECKS))
		{
				...
				{
                    if (output.scriptPubKey == FOUNDER_1_SCRIPT && output.nValue == (int64_t)(14.5 * COIN))
					{
						found_1 = true;
                        continue;
                    }
					
                    if (output.scriptPubKey == FOUNDER_2_SCRIPT && output.nValue == (int64_t)(5 * COIN))
                    {
                        found_2 = true;
                        continue;
                    }
                    ...
                    if (output.scriptPubKey == FOUNDER_5_SCRIPT && output.nValue == (int64_t)(7 * COIN))
                    {
                        found_5 = true;
                        continue;
                    }

                    if (output.scriptPubKey == FOUNDER_6_SCRIPT && output.nValue == (int64_t)(3 * COIN))
                    {
                        found_6 = true;
                        continue;
                    }
            ...
            if (!(found_1 && found_2 && found_3 && found_4 && found_5 && found_6))
			{
                return state.DoS(100, false, REJECT_FOUNDER_REWARD_MISSING,
                                 "CTransaction::CheckTransaction() : team reward missing");
            }
