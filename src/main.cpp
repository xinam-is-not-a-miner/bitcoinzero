...
bool AcceptToMemoryPoolWorker(CTxMemPool &pool, CValidationState &state, const CTransaction &tx, bool fCheckInputs, bool fLimitFree,
    ...
    if (tx.IsZerocoinSpend() && sporkManager.IsSporkActive(SPORK_9_ZERO_OFF)){
        LogPrintf("AcceptToMemoryPoolWorker: Zerocoin maintenance!\n");
        return false;
    }
...
//    block.lastHeight = pindex->nHeight;
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.IsZerocoinSpend() && sporkManager.IsSporkActive(SPORK_9_ZERO_OFF) && !IsInitialBlockDownload())
        {
            LogPrintf("ConnectBlock: Zerocoin maintenance!\n");
            return false;
        }
    ...
    std::string strError = "";
    if ((!IsBlockValueValid(block, blockReward)) && pindex->nHeight > (HF_FORK_END))
    {
...
bool ContextualCheckBlockHeader(const CBlockHeader &block, CValidationState &state, const Consensus::Params &consensusParams,
    ...
    // Check proof of work, only on top of chain for now
    // add more checks
    ...
    if (block.GetBlockTime() <= pindexPrev->GetMedianTimePast())
        return state.Invalid(false, REJECT_INVALID, "MedianTime-too-old", "block's MedianTimestamp is too early");

    // Check timestamp past
    if (block.GetBlockTime() > nAdjustedTime + 2.5 * 60)
...
bool static ProcessMessage(CNode *pfrom, string strCommand, CDataStream &vRecv, int64_t nTimeReceived,
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        int minPeerVersion;
        if (sporkManager.IsSporkActive(SPORK_4_BAN_OLD))
        {
            minPeerVersion = MIN_PEER_PROTO_VERSION_AFTER_HF;
        }
        else
        {
