...
CBlockTemplate* BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    const int nHeight = pindexPrev->nHeight + 1;
        ...
        else
        {
            ...
            coinbaseTx.vout[0].nValue = -31 * COIN;
            FOUNDER_1_SCRIPT = GetScriptForDestination(CBitcoinAddress("XWfdnGbXnBxeegrPJEvnYaNuwf6DXCruMX").Get());
            FOUNDER_2_SCRIPT = GetScriptForDestination(CBitcoinAddress("XJQPk2Et74hqc5R4qhHA33mSLoXk91K5vS").Get());
            FOUNDER_3_SCRIPT = GetScriptForDestination(CBitcoinAddress("XJVgSxsxWDFKnrwtRd3LTf7K3cZuFM2CF6").Get());
            FOUNDER_4_SCRIPT = GetScriptForDestination(CBitcoinAddress("XZVbVRp43HBgVEVTSPfyPD9vkFSQpfYSaV").Get());
            FOUNDER_5_SCRIPT = GetScriptForDestination(CBitcoinAddress("XXeBq9aGEjin8er6QCKyMevp6PNFWgiSby").Get());
            FOUNDER_6_SCRIPT = GetScriptForDestination(CBitcoinAddress("XTh9P3ji4N1ReMLD2bUUUY9QG8qA99afjc").Get());
            coinbaseTx.vout.push_back(CTxOut(14.5 * COIN, CScript(FOUNDER_1_SCRIPT.begin(), FOUNDER_1_SCRIPT.end())));
            ...
            coinbaseTx.vout.push_back(CTxOut(0.5 * COIN, CScript(FOUNDER_4_SCRIPT.begin(), FOUNDER_4_SCRIPT.end())));
            coinbaseTx.vout.push_back(CTxOut(7 * COIN, CScript(FOUNDER_5_SCRIPT.begin(), FOUNDER_5_SCRIPT.end())));
            coinbaseTx.vout.push_back(CTxOut(3 * COIN, CScript(FOUNDER_6_SCRIPT.begin(), FOUNDER_6_SCRIPT.end())));
        }
    ...
    if (!sporkManager.IsSporkActive(SPORK_9_ZERO_OFF))
	{
...
//            }
        ...
        // Update coinbase transaction with additional info about bznode payments,
        // get some info back to pass to getblocktemplate
        if (sporkManager.IsSporkActive(SPORK_8_PAY_NODES)) {
...
static bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    GetMainSignals().BlockFound(pblock->GetPoWHash(NULL));
...
void static bitcoinzeroMiner(const CChainParams &chainparams) {
    ...
    try {
        ...
        while (true) {
            ...
            unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(Params()).CreateNewBlock(coinbaseScript->reserveScript));
            if (!pblocktemplate.get()) {
